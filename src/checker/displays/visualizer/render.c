...
/*   Updated: 2019/09/17 11:27:28 by bharrold         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_b(t_visualizer *v, t_history *history)
{
	...
	while (i >= 0)
	{
		SDL_Rect rect = {STACK_B_X_START,
			STACK_B_Y_START + i * (v->rect_height),
			history->b_state[i] * v->rect_unit_width + 1, v->rect_height};
		SDL_RenderFillRect(v->renderer, &rect);
		i--;
	}
}
